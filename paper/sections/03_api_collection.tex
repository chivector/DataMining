...
本实验不使用全部候选 Prompt 池，而是选取 5 组代表性来源：\path{data/prompts.csv} 作为 baseline，加上重点策略 \path{real_world_archetype_matrix}、\path{halton_space_filling}、\path{dimensionless_ratio_design} 和 \path{constraint_boundary_solver}。每组 300 条，共 1500 条 Prompt。该分层主样本兼顾原始基线、现实原型、参数空间填充、相对比例表达和临界边界样本，在有效控制调用成本的同时全面覆盖主要实验变量。
...
实验流程由四个脚本串联完成。首先运行 \path{scripts/build_experiment_prompts.py}，从选定 Prompt 文件生成 \path{data/experiment_prompts.csv} 并检查 \code{prompt_uid} 唯一性。随后运行 \path{scripts/collect_experiment_responses.py} 调用模型 API，原始输出写入 \path{data/raw/model_responses_experiment.csv}。接着运行 \path{scripts/build_experiment_features.py}，将原始回复转换为 \path{data/processed/behavior_features_experiment.csv}。最后运行 \path{scripts/analyze_experiment.py}，将策略维度和模型维度统计表写入 \path{outputs_experiment/}。

API 调用使用固定采样参数，当前配置为 \code{temperature=0.2}、\code{max_tokens=900}、\code{timeout_seconds=300}、\code{seed=42}、\code{max_retries=3}。正式运行前先进行小规模冒烟测试，确认模型接口可正常访问、回复不为空且最终判断格式能够被正确解析；正式采集阶段则采用并发 workers 分批执行。

\input{figures/collection_pipeline}
...
\end{equation}
而非仅依赖 \code{prompt_id}。由此即使不同策略文件中存在相同的 \code{prompt_id}，在断点续跑时亦不会被误判为已采集。

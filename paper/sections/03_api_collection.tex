...
\end{equation}
其中 \code{prompt_uid} 是后续 API 采集、断点续跑、特征构建和覆盖率检查的全局 Prompt 标识。
...
\path{outputs_experiment/} & 后续策略、模型、扰动和层级统计结果。\\
\bottomrule
\end{tabularx}
\end{table}
...
脚本支持 \code{--resume}、\code{--workers}、\code{--log-every} 和 \code{--flush-every}。其中 \code{--workers} 控制并发数量，\code{--log-every} 控制终端进度打印频率，\code{--flush-every} 控制定期写盘间隔。每次调用都会记录 \code{status}、\code{error_message}、\code{latency_seconds} 和完整 \code{response}。失败调用不会被静默丢弃，而是以 \code{status=failed} 进入原始表，便于后续检查失败是否集中在特定模型、限流或权限问题上。
